...
/**
 * @description Function to get bioethanol total items by ventas totales
...
module.exports.handler = async (event) => {
  try {
    ...
    ventasTotales = await event.pathParameters.ventasTotales;

    if (!ventasTotales) {
      return await bodyResponse(
        BAD_REQUEST_CODE,
        'The ventas totales parameter is required',
      );
    }

    validatePathParam = await validatePathParameters(ventasTotales);

    if (!validatePathParam) {
      return await bodyResponse(
        BAD_REQUEST_CODE,
        `Bad request, check malformed ventas totales ${ventasTotales}`,
      );
    }
    ...
    msgResponse = 'ERROR in get-like-ventas-totales controller function for bioethanol-total.';
