...
/**
 * @description Function to get bioethanol total items by produccion
...
module.exports.handler = async (event) => {
  try {
    ...
    produccion = await event.pathParameters.produccion;

    if (!produccion) {
      return await bodyResponse(
        BAD_REQUEST_CODE,
        'The produccion parameter is required',
      );
    }

    validatePathParam = await validatePathParameters(produccion);

    if (!validatePathParam) {
      return await bodyResponse(
        BAD_REQUEST_CODE,
        `Bad request, check malformed produccion ${produccion}`,
      );
    }
    ...
    msgResponse = 'ERROR in get-like-produccion controller function for bioethanol-total.';
